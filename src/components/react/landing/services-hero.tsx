...
function GradientMesh() {
    return (
        <div className="absolute inset-0">
            ...
            />
            <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: `radial-gradient(circle at 1px 1px, currentColor 1px, transparent 0)', size: '24px 24px'}` }} />
        </div>
    );
}
...
export function ServicesHero({ locale }: ServicesHeroProps) {
    ...
    const y1 = useSpring(useTransform(scrollY, [0, 500], [0, 150]), { stiffness: 100, damping: 20 });
    const y2 = useSpring(useTransform(scrollY, [0, 500], [0, -100]), { stiffness: 100, damping: 20 });
    ...
    return (
        <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
            ...
            <motion.div style={{ y: y1 }} className="absolute top-20 left-10 md:left-20 opacity-20">
                <FloatingShape className="w-16 h-16 border border-blue-400/40 rotate-12 rounded-lg" delay={0} />
            </motion.div>
            <motion.div style={{ y: y2 }} className="absolute bottom-20 right-10 md:right-20 opacity-20">
